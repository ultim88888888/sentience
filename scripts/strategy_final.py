...
Long the top-3 a16z-corpus (A2b council) BULLISH sectors that ALSO clear a trailing-1mo momentum gate;
equal-weight their liquid baskets; hold BTC when none qualify. Monthly rebalance. $10mm notional.
...
Profile (2022-12..2026-03, 39 monthly periods): Sharpe ~1.1 (≈BTC), Jensen alpha ~50%/yr, β~0.8, DD−45%.
HONEST: alpha t≈1.2 — promising, not formally significant at n=39. Deliverable = the mechanism + this
characterization, per the case-study brief."""
...
CAPITAL = 10_000_000
MOM_GATE = 0.10   # trailing-1mo basket momentum bar (10% — top of the monotonic plateau)
TOP_K = 3
...
def weights(t):
    ...
    for sec, conv in a2b_bullish(t).items():
        ...
        if not b:
            continue
        m = _basket_trailing(b, prices, t, 30)
        if m is not None and m >= MOM_GATE:
    ...
    return {s: x / g for s, x in raw.items()}, [c[1] for c in cand]
...
def main():
    ...
    print(f"  (top-{TOP_K} A2b-bullish ∩ mom≥{MOM_GATE:.0%}, BTC fallback, monthly)")
    print("=" * 64)
